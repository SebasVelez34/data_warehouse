const locations = (() => {
        ...
        regions.map(region =>{
            if(region.isActive)
                html += `
                    <li>
                        <a href="#">${region.name}
                            <button class="btn delete" type="button" title="Eliminar" data-action="delete" data-self="${region.id}" data-type="region" data-child="country" data-type-parent="region" data-parent="${region.id}"><i class="ni ni-fat-delete"></i></button>
                            <button class="btn edit" type="button" title="Editar" data-action="edit" data-self="${region.id}" data-type="region" data-child="country" data-type-parent="region" data-parent="${region.id}"><i class="ni ni-settings"></i></button>
                            <button class="btn create" type="button" title="Añadir un país" data-action="create" data-self="${region.id}" data-type="region" data-child="country" data-type-parent="region" data-parent="${region.id}"><i class="ni ni-fat-add"></i></button>
                        </a>
                        ${ region.Countries ? `
                            <ul>
                                ${ region.Countries.map(country => {
                                    if(country.isActive)
                                        return `
                                            <li>
                                                <a href="#">${country.name}
                                                    <button class="btn delete" type="button" title="Eliminar" data-action="delete" data-self="${country.id}" data-type="country" data-parent="${region.id}" data-child="city" data-type-parent="region"><i class="ni ni-fat-delete"></i></button>
                                                    <button class="btn edit" type="button" title="Editar" data-action="edit" data-self="${country.id}" data-type="country" data-parent="${region.id}" data-child="city" data-type-parent="region"><i class="ni ni-settings"></i></button>
                                                    <button class="btn create" type="button" title="Añadir una ciudad" data-action="create" data-self="${country.id}" data-type="country" data-parent="${region.id}" data-child="city" data-type-parent="region"><i class="ni ni-fat-add"></i></button>
                                                </a>
                                                ${ country.Cities ? `
                                                    <ul>
    ...
    middelware =  () => {
        $(document).on("click","button.create,button.edit,button.delete",async function() {
            ...
            if(action === "create"){
                ...
                showConfirm({
                    callbackConfirm: async () => {
                        ...
                        const response = await deleteLocation({type, id, data: { parent }});
                ...
                await edit({type,id});
                $("#modal-form").modal("show");
                console.log("El values es:", $("#locationParent").val(),parent);
                $("#locationParent").attr("name",`${dataTypeParent}_id`).val(parent);
                console.log("El values es:", $("#locationParent").val());
    ...
    titleLocation = (title = true) => {
        title = title === "region" ? "País" : title === "country" ? "Ciudad" : title;
    ...
    edit = async ({type, id}) => {
        await fetch(`${API_URL}/location/${type}/${id}`)
			.then((data) => data.json())
			.then((data) => {
				$("#modal-form").modal("show");
